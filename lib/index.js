...
var DEFAULT_DPIROOT = 'ws://dpi.organiq.io';
 ...
 *  (3) ORGANIQ_DPIROOT, ORGANIQ_APIKEY_ID, ORGANIQ_APIKEY_SECRET and
 *    ORGANIQ_NAMESPACE environment variables
 *
...
function Organiq(options) {
  ...
  if (!apiRoot || !apiKeyId || !apiKeySecret || !!namespace) {
    if (fs && fs.existsSync !== undefined && fs.existsSync(optionsPath)) {
      ...
      apiRoot = apiRoot || config['dpiRoot'];
      namespace = namespace || config['namespace'];
    }
  }

  apiRoot = apiRoot || process.env['ORGANIQ_DPIROOT'] || DEFAULT_DPIROOT;
  ...
  var ws = new WebSocket(apiRoot, null,
